...
namespace Project_K.ViewModel
{
    public class GenerateReportVM : INotifyPropertyChanged
    {
        ...
        public ICommand EmailsCommand => new RelayCommand(DownloadEmails);
        public List<string> Classes { get; set; }
        public string SelectedClass { get; set; }

        public GenerateReportVM()
        {
            Meters = new List<string> {"All Meters" }.Concat(GetMeterNames()).ToList();
            ...
            {
                SelectedMeter = "All Meters"; // Or handle appropriately 
            }
            else
            {
                SelectedMeter = "All Meters";
            }
        }
        ...
        private void AddEmail()
        {
            if (!string.IsNullOrWhiteSpace(EmailAddress) && !EmailAddresses.Contains(EmailAddress))
            {
               // Trace.WriteLine(EmailAddress.Length);
                ...
                MessageBox.Show($"Email added", "Success", MessageBoxButton.OK, MessageBoxImage.Information);
            }
        }
        ...
        private void RemoveEmail()
        {
            if (EmailAddresses.Contains(EmailAddress))
            {
                ...
                MessageBox.Show($"Email Removed", "Success", MessageBoxButton.OK, MessageBoxImage.Information);
        ...
        private void DownloadEmails()
        {
            try
            {
                ...
                string fileName = GetFileName() + "\\" + "EmailAddresses.csv";

                // Create a FileStream and StreamWriter for writing to the CSV
                ...
                using (StreamWriter writer = new StreamWriter(stream, Encoding.UTF8))
                {
                    ...
                    // Write each email address from the database
                ...
                MessageBox.Show($"Emails have been downloaded to {fileName}", "Download Successful", MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                // Handle any errors during the file creation
                MessageBox.Show($"Error occurred: {ex.Message}", "Download Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void DownloadReport()
        {
            ...
            string selectedMeter = SelectedMeter;
            ...
            try
            {
                if (selectedClass == "MeterData")
                {
                    ...
                    if (success >= 0)
                    {
                            ...
                            foreach (DateTime date in queryDates)
                            {
                                ...
                                List<string> meters = (SelectedMeter == "All Meters") ? Meters.Where(m => m != "SelectMeter" && m != "All Meters").ToList() : new List<string> { SelectedMeter };
                                foreach (string meterName in meters)
                                {
                                    var q = connection.Table<MeterData>().Where(v => v.Date.StartsWith(qu) && v.MeterName == meterName);
                    ...
                    if (success >= 0)
                    {
                        ...
                        foreach (string meterName in meters)
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    qu = date.ToString("dd-MM-yyyy");
                                    var q = connection.Table<MinMaxValues>().Where(v => v.Date.StartsWith(qu) && v.MeterName == meterName);
                ...
                else if (selectedClass == "EnergyData")
                {
                    ...
                    writer2 = new StreamWriter(stream2, Encoding.UTF8); { writer2.WriteLine("METERNAME,DATE,Energy");}
        ...
        private void DownloadReportForPreviousDay1()
        {
            ...
            csvContent.AppendLine("METERNAME,DATE,TIME,VOLTAGE,CURRENT,PF,Energy");
            ...
            using (SQLiteConnection connection = new SQLiteConnection(App.databasePath))
            {
                ...
                foreach (MeterData report in query)
                {
                    if (report != null)
                    {
                        ...
                        csvContent.AppendLine($"{report.MeterName},{'*' + report.Date},{'*' + report.Timestamp},{report.Voltage},{report.Current},{report.PF},{report.KWH}");
                    }
                } 
            }
