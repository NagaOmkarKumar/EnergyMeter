...
namespace Project_K.ViewModel
{
    public class RoboWorksVM : ObservableObject
    {
                                ...
                                if (true)
                            {
                               ...
                               // foreach (var word in words) { Trace.WriteLine(word); }
                                ...
                                MeterData meterData = new MeterData
                                {
                                    ...
                                    Voltage = Math.Round(Convert.ToDouble(words[0]), 2),
                                    Current = Math.Round(Convert.ToDouble(words[1]), 2),
                                    PF = Math.Round(Convert.ToDouble(words[2]), 2),
                                    KWH = Math.Round(Convert.ToDouble(words[3]), 2),
                                    KVA = Math.Round(Convert.ToDouble(words[4]), 2),
                                ...
                                await _firebaseService.StoreToFirestore(meterData.MeterName, meterData.Voltage, meterData.Current, meterData.PF, meterData.KWH, meterData.KVA);
                                if (savedData != null)
                                    {
                                ...
                                if (meterData.PF >= savedData.MaxPF) { savedData.MaxPF = meterData.PF; savedData.MaxPFTime = DateTime.Now.ToString(" HH:mm"); }  
                                    ...
                                    if (meterData.PF >= 0.05) { if (meterData.PF <= savedData.MinPF) { savedData.MinPF = meterData.PF; savedData.MinPFTime = DateTime.Now.ToString(" HH:mm"); } }
        ...
        {
            // Create a new timer that ticks every minute (60000 ms)
            dateCheckTimer = new System.Timers.Timer(60000);
            dateCheckTimer.Elapsed += CheckDateChange;  // Event handler for the timer tick
            dateCheckTimer.AutoReset = true;  // Repeating the check every minute
            dateCheckTimer.Start();  // Start the timer
        }
        private void CheckDateChange(object sender, System.Timers.ElapsedEventArgs e)
        {
           
            if (Today != DateTime.Today.ToString("dd-MM-yyyy"))
            {
                {
                    Today = DateTime.Today.ToString("dd-MM-yyyy");
                    Downloaded = false;
